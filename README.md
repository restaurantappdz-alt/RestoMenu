...
├── tools/layout-shots/  # Shot regeneration harness (Playwright)
└── README.md
```

---

> **Note:** The subscription-guard feature (TV-side expiration enforcement with RTDB clock offset) is implemented in `tv-display/src/subscriptionGuard.js`. The UI to set `expiresAt` on the Firestore `config/display` document lives in the **RestoMenuAdminMobile** Flutter app (super-admin dashboard) — until an admin sets an expiry date, the TV will always show a black screen (`no_expiration`). Since 2026-08-05 the expiry is ALSO mirrored to the RTDB node `subscription/{restaurantId}/expiresAt` (written only with the super-admin claim; read denied by rules once `now` passes it), so the server clock — not the device clock — decides expiry. See [subscription guard design doc](docs/superpowers/specs/2026-07-29-subscription-guard-design.md) for the full spec.
...
The thumbnails are generated by the harness in `tools/layout-shots/` (Playwright + the `?demo=1` mode above). Run it after adding or changing a layout:

```bash
cd tv-display && npm run build && npm run preview   # terminal 1
cd tools/layout-shots && npm install && npm run shoot  # terminal 2
```

See `tools/layout-shots/README.md` for details.
...
Run the layout shots harness (`tools/layout-shots/`, see its README) to regenerate the 10 JPGs into `tv-display/public/layout-shots/`, then add the entry to `tv-display/public/layout-shots/layouts.json` so the mobile app picks it up.

### Step 6: Preview locally

- **Screenshot preview:** `http://localhost:5174/RestoMenu/?demo=1&layout=yourName` (fixture data, no Firebase)
