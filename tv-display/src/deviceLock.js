...
// enforce "first device wins" server-side: a lease can only be written when
// it does not exist yet, or by the same deviceId that already owns it.
//
// Each screen (TV1, TV2, ...) has its own lease scope so multiple screens of
// the same restaurant can run simultaneously, while one link still binds to
// a single device at a time. Missing ?s= falls back to scope 'default'.
...
/**
 * Delete the lease this device currently holds. Used when the owner sends a
 * release signal so another device can claim this screen.
 */
 ...
 * The new connection always wins over an old/inactive one.
 */
