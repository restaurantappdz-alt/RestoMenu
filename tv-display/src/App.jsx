...
/* ── Cutlery wallpaper — spaced-out white silhouettes, no overlap ── */
function RestaurantPattern() {
  const items = [
    // ── Large focal accents — 130-180px, 4% (one per zone) ──
    ...
    // ── Medium stamps — 70-100px, 5% (between large zones) ──
    ...
    // ── Small accents — 32-56px, 6% (edges and sparse scatter) ──
  ...
  return (
    <span className="item-connector" aria-hidden="true">
      <svg viewBox="0 0 300 14" preserveAspectRatio="none" fill="none" strokeLinecap="round">
        <path
          d="M0,7 C16,2 32,12 48,7 C64,2 80,12 96,7 C112,2 128,12 144,7 C160,2 176,12 192,7 C208,2 224,12 240,7 C256,2 272,12 288,7 C300,7 300,7 300,7"
          stroke="rgba(255,183,3,0.18)"
          strokeWidth="1.4"
        />
        <path
          d="M0,9 C18,4 36,14 54,9 C72,4 90,14 108,9 C126,4 144,14 162,9 C180,4 198,14 216,9 C234,4 252,14 270,9 C288,4 300,9 300,9"
          stroke="rgba(255,81,0,0.10)"
          strokeWidth="0.9"
        />
        <path
          d="M2,7 C20,1 38,11 56,7 C74,1 92,11 110,7 C128,1 146,11 164,7 C182,1 200,11 218,7 C236,1 254,11 290,7"
...
export default function App() {
  ...
  useEffect(() => {
    const unsubConfig = onSnapshot(doc(db, 'config', 'display'), (snap) => {
      ...
      if (!newId) { setWaiting(true); setLoading(false); setMenu(null); setActiveMenuId(null); return }
      setWaiting(false); setActiveMenuId(newId)
  ...
  useEffect(() => {
    ...
    const unsubMenu = onSnapshot(doc(db, 'menus', activeMenuId), (snap) => {
      if (snap.exists()) { const data = { id: snap.id, ...snap.data() }; setMenu(data); saveCache(data); setOffline(false) }
      else setMenu(null)
      setLoading(false)
    }, () => { setOffline(true); setLoading(false) })
    return () => unsubMenu()
      ...
      <div className="content-canvas">
        ...
        <div className="header-row">
          <div className="animate-pop-in">
            <h1 className="font-display text-white leading-[1.05] tracking-tight text-display-xl">
              Sandwich<br /><span className="text-brand-honey">M'delda</span>
            </h1>
            <p className="text-white/60 mt-3 font-heading font-semibold tracking-wide text-subtitle">
              Sans frites mais charg&eacute; de viandes
            </p>
          </div>
        ...
        <div className="content-inner">
          <div className="flex-1 flex gap-6 xl:gap-8 min-h-0 mt-2 xl:mt-4">
                    ...
                    return (
                      <div key={ci}>
                        <div className="relative inline-block mb-5 xl:mb-6 animate-fade-in-up">
                          <h2 className="font-heading font-black text-white uppercase tracking-[0.08em] text-category">
                            {cat.name}
                          </h2>
                          <BrushStroke />
                        </div>
                        ...
                        <div>
                          {items.map((item, j) => (
                            <div key={j} className="menu-item animate-cascade-item" style={{ animationDelay: `${j * 0.1}s` }}>
                              <span className="item-name font-heading font-bold text-white leading-tight text-item">
                                {item.name}
                              </span>
                              <SketchyConnector />
                              <div className="item-price">
                                <span className="font-heading font-black text-brand-honey leading-tight tabular-nums text-price">
                                  {item.price}
                                </span>
                                <span className="font-heading font-bold ml-2 text-price-currency" style={{ color: `rgba(255, 183, 3, 0.6)` }}>
                                  D.A
                                </span>
                              </div>
                            </div>
            ...
            {/* RIGHT: Addon — top-aligned with first item */}
            {allAddons.length > 0 && (
                ...
                <div className="flex flex-col items-center text-center mt-2">
                  ...
                  {allAddons.map((addon, j) => (
                    <p key={j} className="font-heading font-black text-brand-honey text-addon-price animate-cascade-item" style={{ animationDelay: `${(j + 0.5) * 0.1}s` }}>
                      {addon.price}
                      <span className="font-heading font-bold ml-2 text-addon-currency" style={{ color: `rgba(255, 183, 3, 0.55)` }}>
                        D.A
                      </span>
                    </p>
                  ))}
                  ...
                  <div className="mt-7 px-6 py-2 bg-white/8 rounded-full border border-white/10">
                    <span className="font-heading font-semibold text-white/60 text-addon-prompt">
                      + Add to your order
                    </span>
                  </div>
                </div>
