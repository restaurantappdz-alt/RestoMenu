# Photo Menu Layout — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a new "Photo Menu" TV layout with hero photo, item descriptions, and tags.

**Architecture:** New layout component in tv-display that reads optional fields from menu/items. No admin dashboard changes. Data model stays backward-compatible — all new fields are optional.

**Tech Stack:** React 18, Tailwind CSS, Vite, Firebase Firestore

## Global Constraints

- No admin-dashboard files modified under any circumstances
- All new data fields must be optional (backward compatible)
- Fonts loaded via `@import url(...)` in `<style>` tags (no npm packages)
- All font sizes use `clamp()` for TV resolution scaling
- Staggered animation delays on items (`${j * 0.1}s`)
- Build must pass: `cd tv-display && npm run build`

---

### Task 1: Create LayoutPhotoMenu.jsx

**Files:**
- Create: `tv-display/src/layouts/LayoutPhotoMenu.jsx`

**Interfaces:**
- Consumes: Props `{ categories, allAddons, offline, menu, title }` (same as all layouts)
  - `categories`: `Array<{ name, items: [{ name, price, description?, tag? }], addons? }>`
  - `allAddons`: `Array<{ name, price }>` (flattened from categories)
  - `offline`: `boolean`
  - `menu`: `object` with optional fields `heroImageUrl`, `heroName`, `heroDescription`, `heroLabel`, `heroPrice`, `allergyNote`, `pricingNote`, `businessHours`, `tagline`, `currency`
  - `title`: `string`
- Produces: Exported `capabilities` object + default exported component
...
export default function LayoutPhotoMenu({ categories, allAddons, offline, menu, title }) {
  ...
  return (
    ...
    >
      ...
      {/* OLED burn-in prevention — pixel shift */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
...
```bash
Test-Path -LiteralPath "tv-display/src/layouts/LayoutPhotoMenu.jsx"
```
...
### Task 2: Register LayoutPhotoMenu in tv-display layouts/index.js

**Files:**
- Modify: `tv-display/src/layouts/index.js`

**Interfaces:**
- Consumes: `LayoutPhotoMenu` default export from Task 1
- Produces: `photoMenu` entry in `layouts` object

- [ ] **Step 1: Read the current registry**

```bash
Get-Content -Path "tv-display/src/layouts/index.js"
```

- [ ] **Step 2: Add import and registry entry**

Edit `tv-display/src/layouts/index.js`:
...
Insert in the `layouts` object (before the closing `}`):
```js
...
Expected: The file shows the new import and the `photoMenu` registry entry.
...
Expected: Exit code 0, no errors. Output should end with something like `✓ built in X.Xs`.

- [ ] **Step 2: Verify no admin-dashboard files were modified**

```bash
cd "C:\Users\LAPWIZ\Desktop\coding\APP MENU\RestoMenuWeb"
git diff --name-only
```

Expected: Only `tv-display/src/layouts/LayoutPhotoMenu.jsx` and `tv-display/src/layouts/index.js` appear. No `admin-dashboard/` files.
